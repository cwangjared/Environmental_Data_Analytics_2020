...
vector1[3] # the third number element of vector 1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # by column if not specified
matrix1
matrix2 <- matrix(c(1,2,3,4,5), nrow = 5, ncol = 4, byrow = TRUE) #
matrix2
...
f <- c(TRUE, TRUE, TRUE, FALSE) # logic vector
dataframe1 <- data.frame(d,e,f) #
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # semicolon can combine two commands in one line

dataframe1[1:2,] # 
df01 <- dataframe1[c("ID","Passed")] # 
df01
dataframe1$ID
...
> Answer: the environment provides information about different data types, i.e. int, chr, num, and logi.

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: the three column names appear for choosing.
...
```{r}
far_low  <- c(31,29,43,24,23,23,26,30,42,41) 
far_high <- c(62,45,50,50,40,40,45,52,54,58)

round(8.613, digits = 1)
```

Now, create two additional vectors that include the ten-day forecast for the high and low temperatures in Celsius.

```{r}
cel_low  <- c((far_low - 32)*5/9)

cel_high <- c((far_high - 32)*5/9)
...
df01 <- data.frame(far_low, far_high, cel_low, cel_high) #
names(df01) <- c("Low Temperature (F)","High Temperature (F)","Low Temperature (C)", "High Temperature (C)")
View(df01)
```
